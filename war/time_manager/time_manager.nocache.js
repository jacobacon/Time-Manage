...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "time_manager" because\n';
  ...
  $wnd.__gwt_activeModules['time_manager'] = {
    'moduleName' : 'time_manager',
    'bindings' : function() {
      return {};
    }
  };
 ...
 * "/time_manager.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['time_manager'].superdevmode = true;

  var expectedSuffix = '/time_manager.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + time_manager + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:time_manager';
  var devModeSessionKey = '__gwtDevModeSession:time_manager';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/time_manager/time_manager.nocache.js';
  ...
  $wnd.__gwt_activeModules['time_manager'].canRedirect = true;
  $wnd.__gwt_activeModules['time_manager'].superdevmode = true;
  ...
  css = (moduleIdx == 1 ? css : '') +
    ".gwt-DevModeModule-" + moduleIdx + ".gwt-DevModeCompiling div:before{" +
      "content:'COMPILING time_manager';" +
      "font-size:24px;" +
      "color:#d2d9ee;" +
    "}";
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/time_manager?user.agent=' + ua + '&_callback=' + callback;
